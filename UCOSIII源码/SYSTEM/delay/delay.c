...
void delay_init()
{
	...
	fac_us 	= SystemCoreClock/8000000;		//为系统时钟的1/8(为9)
#if SYSTEM_SUPPORT_OS  									//如果需要支持OS.
	reload  = SystemCoreClock/8000000;		//每秒钟的计数次数 单位为K(9)   
...
{		
	...
	SysTick->CTRL|=SysTick_CTRL_ENABLE_Msk ;	//开始倒数（使能系统时钟）	  
	do
	{
		temp = SysTick->CTRL;
	}
	while((temp&0x01)&&!(temp&(1<<16)));			//等待时间到达   
	SysTick->CTRL &= ~SysTick_CTRL_ENABLE_Msk;//关闭计数器（不是一个死循环）
	SysTick->VAL =0x00;      					 				//清空计数器	 
}
