...
//任务控制块
struct os_tcb {
    CPU_STK             *StkPtr; /*指向当前栈顶*/           /* Pointer to current top of stack                        */
    ...
    OS_PRIO              Prio;      /*当前任务的优先级*/   	/* Task priority (0 == highest)                           */
    CPU_STK_SIZE         StkSize;                           /* Size of task stack (in number of stack elements)       */
    OS_OPT               Opt;  /*任务创建时传递的数值*/    	/* Task options as passed by OSTaskCreate()               */
